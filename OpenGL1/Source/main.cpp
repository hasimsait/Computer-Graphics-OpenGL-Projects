...
int main(void) {
	...
	/* Configure OpenGL */
	glClearColor(0, 0, 0, 0.1f);//transparent
	glEnable(GL_DEPTH_TEST);

	/* Creating OpenGL objects */

	VAO sphereVAO = NewShapeVAO(glm::dvec3(0, 0, 0.0), 1, -10, 50, 50);//SphereVAO(glm::dvec3(0, 0, 0.0),1, 16, 16);
	...
	GLuint program = CreateProgramFromSources(
		...
		void main(){
			...
			vec3 static_light_color= vec3(110,180,199)/255;
	...
	auto surface_color = glGetUniformLocation(program, "u_surface_color");
	auto chasing_pos = glm::vec3(0);
	int time = 5000;
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//transform = glm::translate(transform, glm::vec3(mouse_position));
		transform = glm::rotate(transform, glm::radians(-20.f), glm::vec3(1, 0, 0));
		transform = glm::scale(transform, glm::vec3(0.5));
		transform = glm::rotate(transform, glm::radians(float(time*time / 1000)), glm::vec3(0, 1, 0));
		transform = glm::rotate(transform, glm::radians((5000 - time) / 100.f), glm::vec3(1, 0, 1));//increase as it slows down 50.f final 0->50
		glUniformMatrix4fv(transform_location, 1, GL_FALSE, glm::value_ptr(transform));
		glUniform3fv(surface_color, 1, glm::value_ptr(glm::vec3(0.5)));
		glBindVertexArray(sphereVAO.id);
		glDrawElements(GL_TRIANGLES, sphereVAO.element_array_count, GL_UNSIGNED_INT, NULL);
		transform = glm::rotate(glm::mat4(0), glm::radians(0.f), glm::vec3(0, 1, 0));
		glUniformMatrix4fv(transform_location, 1, GL_FALSE, glm::value_ptr(transform));
