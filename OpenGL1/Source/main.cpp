...
int main(void) {
	...
	VAO sphereVAO = SphereVAO(glm::dvec3(0, 0, 0.0),1, 16, 16);
		...
		for (int i = 0; i < 2; i++) {
			...
			case 0:
				...
				if(glm::distance(mouse_position,chasing_pos)>0.6)
					glUniform3fv(surface_color, 1, glm::value_ptr(glm::vec3(0, 1, 0)));
				else 
					glUniform3fv(surface_color, 1, glm::value_ptr(glm::vec3(1, 0, 0)));
			...
			case 1:
				...
				transform = glm::translate(transform, chasing_pos);
				transform = glm::scale(transform, glm::vec3(0.3));
				glUniformMatrix4fv(transform_location, 1, GL_FALSE, glm::value_ptr(transform));
